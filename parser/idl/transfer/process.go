...
func (this *walkListener) EnterGolang_import_(ctx *parser.Golang_import_Context) {
	...
	line := ""
	if aliasNode != nil {
		line = fmt.Sprintf("%s %s", trimDoubleQuote(aliasNode.LITERAL().GetText()), ctx.LITERAL().GetText())
	} else {
		line = fmt.Sprintf("%s", ctx.LITERAL().GetText())
	}
	if ctx.Golang_import_hint().GetText() == "SVC_HINT" || ctx.Golang_import_hint().GetText() == "ALL_HINT" {
		this.def.GoSvcImports = append(this.def.GoSvcImports, line)
	}
	if ctx.Golang_import_hint().GetText() == "STRUCT_HINT" || ctx.Golang_import_hint().GetText() == "ALL_HINT" {
		this.def.GoStructImports = append(this.def.GoStructImports, line)
	}
}
